				...
				<div id="board">
					<div id="list">
						...
						<table>
							...
							<tbody>
								<c:forEach items="${requestScope.pMap.tboardList}"  var="tboard">
									<tr>
										<td>${tboard.no}</td>
										<td class="text-left"><a href="#">${tboard.title}</a></td>
										<td>${tboard.name}</td>
										<td>${tboard.hit}</td>
						...
						<div id="paging">
							<ul>
								<c:if test = "${requestScope.pMap.prev}">
									<li><a href="${pageContext.request.contextPath}/tboard/list3?crtpage=${requestScope.pMap.startPageBtnNo}">◀</a></li>
								</c:if>
								...
								<c:forEach begin = "${requestScope.pMap.startPageBtnNo}" 
									...
									<c:choose>
										...
										<c:otherwise>
											<li><a href="${pageContext.request.contextPath}/tboard/list3?crtpage=${page}">${page}</a></li>
										</c:otherwise>
									</c:choose>
								...
								<c:if test = "${requestScope.pMap.next}">
									<li><a href="${pageContext.request.contextPath}/tboard/list3?crtpage=${requestScope.pMap.endPageBtnNo}">▶</a></li>
								</c:if>
							</ul>
